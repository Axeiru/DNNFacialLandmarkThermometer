...
def raw_to_8bit(data):
    # Preprocess raw thermal pixel data: normalized and bitshifted into a single byte to be represented as black and white image
...
def displayTemperature(img, tempKelvin, position, color):
    # Label temperature onto image
    val = ctof(tempKelvin)
    output = cv2.putText(img, "{0:.1f} degF".format(val), position, cv2.FONT_HERSHEY_COMPLEX, 0.75, color, 2)
    x, y = position
    output = cv2.line(output, (x - 2, y), (x + 2, y), color, 1)
    output = cv2.line(output, (x, y - 2), (x, y + 2), color, 1)
    return output


def displayFaceTemperature(img, tempKelvin, position, color, size):
...
def detectFaces(self, visible_image, thermal_image, thermal_data):
    ...
    for i in range(0, detections.shape[2]):
        ...
        cv2.putText(visible_image, text, (startX, y), cv2.FONT_HERSHEY_COMPLEX , 0.45, (0, 0, 255), 2)
            ...
            if 0 < x < x_bound and 0 < y < y_bound:
                try:
                    ...
                    if n == 39 or n == 42:
                        ...
                        visible_image = displayFaceTemperature(visible_image, raw_value, (x,y), (50, 255, 87), 1)
                        thermal_image = displayFaceTemperature(thermal_image, raw_value, (x,y), (0, 0, 0), 3)
...
class GUI:
    def __init__(
        ...
        window,
        windowTitle,
        visibleCam = (gstreamer_pipeline(flip_method=2), cv2.CAP_GSTREAMER, False),
        ...
        self.canvas = tkinter.Canvas(window, width=1920, height=359)
    ...
    def readMousePosition(self):
        ...
        relativeX = (pointerX - windowX) % 640
        relativeY = pointerY - windowY
        if relativeY > 359:
            relativeY = 359
    ...
    def update(self):
        # Main Tkinter window update logic
        ...
        if thermal_ret and visible_ret:
            ...
            overlayed_view = cv2.addWeighted(thermal_image, (self.opacitySlider.get()/100), visibleImageDetectedFaces, 1-(self.opacitySlider.get()/100), 0)
            output_frame = np.concatenate((visibleImageDetectedFaces, overlayed_view, thermal_image), axis=1)
            self.currentFrame = output_frame
            self.outputPhoto = PIL.ImageTk.PhotoImage(PIL.Image.fromarray(output_frame))
            if self.recording:
                self.outputRecording.write(cv2.cvtColor(output_frame,cv2.COLOR_RGB2BGR))
        ...
        if self.vid.isOpened():
            ...
            if ret:
                if self.camera_settings[2]:
                    thermal_data_values = cv2.flip((cv2.resize(frame[:, :], (640, 480))[59:419, :]), 1)

                    single_channel_thermal_img = cv2.cvtColor(raw_to_8bit(thermal_data_values.copy()), cv2.COLOR_BGR2GRAY)
                    minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(thermal_data_values)

                    colormap = self.customColorMap
                    thermal_img = (colormap(single_channel_thermal_img) * 2**8).astype(np.uint8)[:,:,:3]

                    thermal_img = cv2.cvtColor(thermal_img, cv2.COLOR_RGBA2RGB)

                    thermal_img = displayTemperature(thermal_img, minVal, minLoc, (0, 0, 255))
                    thermal_img = displayTemperature(thermal_img, maxVal, maxLoc, (255, 0, 0))

                    return (ret, thermal_img, thermal_data_values)
